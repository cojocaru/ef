...
Ext.ux.ItemSelector = Ext.extend(Ext.form.Field,  {
    ...
    hideNavIcons:false,
    imagePath:"",
    iconUp:"/images/icons/buttons/up2.gif",
    iconDown:"/images/icons/buttons/down2.gif",
    iconLeft:"/images/icons/buttons/left2.gif",
    iconRight:"/images/icons/buttons/right2.gif",
    iconTop:"/images/icons/buttons/top2.gif",
    iconBottom:"/images/icons/buttons/bottom2.gif",
    drawUpIcon:true,
    drawDownIcon:true,
    drawLeftIcon:true,
    drawRightIcon:true,
    ...
    onRender: function(ct, position){
        ...
        var el=icons.getEl();
        if (!this.toSortField) {
            this.toTopIcon = el.createChild({'class': 'x-form-multiselect-top', tag:'img', src:this.iconTop, style:{cursor:'pointer', margin:'2px'}});
            el.createChild({tag: 'br'});
            this.upIcon = el.createChild({'class': 'x-form-multiselect-up', tag:'img', src:this.iconUp, style:{cursor:'pointer', margin:'2px'}});
            el.createChild({tag: 'br'});
        }
        this.addIcon = el.createChild({'class': 'x-form-multiselect-add', tag:'img', src:this.switchToFrom?this.iconLeft:this.iconRight, style:{cursor:'pointer', margin:'2px'}});
        el.createChild({tag: 'br'});
        this.removeIcon = el.createChild({'class': 'x-form-multiselect-rem', tag:'img', src:this.switchToFrom?this.iconRight:this.iconLeft, style:{cursor:'pointer', margin:'2px'}});
        el.createChild({tag: 'br'});
        if (!this.toSortField) {
            this.downIcon = el.createChild({'class': 'x-form-multiselect-down', tag:'img', src:this.iconDown, style:{cursor:'pointer', margin:'2px'}});
            el.createChild({tag: 'br'});
            this.toBottomIcon = el.createChild({'class': 'x-form-multiselect-bottom', tag:'img', src:this.iconBottom, style:{cursor:'pointer', margin:'2px'}});
        }
        if (!this.readOnly) {
            if (!this.toSortField) {
    ...
    fromTo : function() {
        ...
        if (selectionsArray.length > 0) {
			...
			this.toMultiselect.view.store.fireEvent('recordsAdded', records);
        }
        this.toMultiselect.view.refresh();
    ...
    toFrom : function() {
        ...
        if (selectionsArray.length > 0) {
			...
			this.toMultiselect.view.store.fireEvent('recordsRemoved', records);
        }
        this.fromMultiselect.view.refresh();
        this.toMultiselect.view.refresh();
    ...
    onRowDblClick : function(vw, index, node, e) {
		return; // this disables doble click
		this.fireEvent('recordDblClick', vw, index, node, e);
    ...
    reset: function(){
		...
		this.toMultiselect.view.store.fireEvent('recordsCleared');
    }
	// new method
	,
	setValue: function(val) {
		...
		for(i = 0; i < val.length; i++) {
			...
			rec = this.fromStore.find('id', new RegExp(rec), false );
